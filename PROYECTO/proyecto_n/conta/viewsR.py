...
from .models import Proyecto, Contaduria_regi, Usuarios
...
def registrarR(request):
    if request.method == 'POST':
        ...
        contadurias = Contaduria_regi.objects.all()
        return render(request, 'contabilidad/registrarR.html', {'contadurias': contadurias})

def total(request):
    suma_total = Contaduria_regi.objects.aggregate(
        total_registro=Sum('valor_registro'), 
    )

    total_asume = (suma_total['total_registro'] or 0) + \
                  (suma_total[''] or 0)
    
    contadurias = Contaduria_regi.objects.all()
    return render(request, 'registrarR.html', {'total_asume': total_asume, 'contadurias': contadurias})


def eliminarR(request):
    if request.method == 'POST':
        numero_registro = request.POST.get('numero_registro')
        contaduria = Contaduria_regi.objects.filter(id_contaduria=numero_registro).first()
        
        if contaduria:
            contaduria.delete()
            return redirect('Eregistro')
        else:
            error_message = 'El registro no existe'
            contadurias = Contaduria_regi.objects.all()
            return render(request, 'contabilidad/eliminarR.html', {'contadurias': contadurias, 'error_message': error_message})
    else:
        contadurias = Contaduria_regi.objects.all()
        return render(request, 'contabilidad/eliminarR.html', {'contadurias': contadurias})

def exportar_excel_regi(request):
    ...
    response['Content-Disposition'] = 'attachment; filename="contaduria_registro.xls"'
    ...
    worksheet.write_merge(16, 16, 0, 7, 'Por concepto del impuesto de registro')
    ...
    columns = ['ITEM', 'Numero de escritura', 'Registro', 'Deposito cliente', 'Total asume C. B']
    ...
    for contaduria in contadurias:
        ...
        worksheet.write(row_num, 2, contaduria.valor_registro, data_style)
